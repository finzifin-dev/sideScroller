...
var CHAR_DEFS = {
  finGuy: {
    ...
    x: 0, y: 0, speed: 3, width: 36, height: 58,
...
function spawnPartyAtGate(side /* "top"|"bottom" */) {
  var g = (side === "top") ? EXIT.top : EXIT.bottom;
  if (!g) return;
  ...
  for (var i = 0; i < party.length; i++) {
    var w = party[i]; if (!w || !w.sprite) continue;

    var s = w.sprite;
    var pad = (side === "top") ? - (s.height / 2) - 2
                               : + (s.height / 2) + 2;

    // simple formation (center, then L/R pairs)
    var dx = 0, dy = 0;
    if (i > 0) {
      var row  = Math.floor(i / 2);
      var sign = (i % 2 === 1) ? -1 : +1; // odd left, even right
      dx = sign * (24 + row * 14);
      dy = -12 * row;
    }
    ...
    // clamp into gate window just in case
    var r = (s.width || TILE_PX) / 2;
...
function goToStage(nextIndex, enteredFrom /* "top"|"bottom" */) {
  ...
  // 1) Build new stage (EXIT is now for the new stage)
  buildStage(nextIndex);

  // 2) Place the party at the *opposite* gate in the new stage
  //    (spawnAtGate for one, or spawnPartyAtGate if you have that)
  spawnPartyAtGate(enteredFrom === "top" ? "bottom" : "top");

  // 3) Stamp the party’s home stage to the new CURRENT_STAGE
  updatePartyStageIndex(CURRENT_STAGE);

  // 4) One clean visibility pass (NPCs & non-party followers)
  syncNPCVisibilityToStage();
}
